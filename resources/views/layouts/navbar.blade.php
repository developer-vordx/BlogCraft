            ...
            <div class="hidden md:flex items-center space-x-6">
                <a href="{{ route('dashboard') }}"
                   class="text-sm font-medium px-3 py-2 rounded-md transition
                          {{ $currentRoute === 'dashboard' ? 'bg-orange-100 text-orange-600' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100' }}">
                    Dashboard
                </a>
                <a href="{{ route('invoice.create') }}"
                   class="text-sm font-medium px-3 py-2 rounded-md transition
                          {{ $currentRoute === 'invoice.create' ? 'bg-orange-100 text-orange-600' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100' }}">
                    Create Invoice
                </a>
                <a href="{{ route('user.verification') }}"
                   class="text-sm font-medium px-3 py-2 rounded-md transition
                          {{ $currentRoute === 'user.verification' ? 'bg-orange-100 text-orange-600' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100' }}">
                    ID Verification
                </a>
                <button id="logoutBtn"
                        class="text-sm font-medium px-3 py-2 rounded-md text-gray-600 hover:text-red-600 hover:bg-gray-100 transition">
                    Logout
                </button>
            </div>
